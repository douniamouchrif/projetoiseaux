...
```{r}
palette <- colorNumeric(palette = "plasma",domain = table_jointe_tbl$MOS11)
...
# Convertir la colonne "Couvert_Nuageux" en facteur
data_join$Couvert_Nuageux <- factor(data_join$Couvert_Nuageux, levels = c("0-25%", "25-50%", "50-75%", "75-100%","100,00%"))

# Créer un résumé du nombre d'espèces par niveau de couverture nuageuse
res_couverture_nuageuse <- data_join %>%
  group_by(Couvert_Nuageux) %>%
  summarise(Nb_espece = n_distinct(Nom_Latin), .groups = 'drop')

# Créer le graphique
ggplot(data = res_couverture_nuageuse, aes(x = Couvert_Nuageux, y = Nb_espece)) +
  geom_bar(stat = "identity", fill = "pink") +
