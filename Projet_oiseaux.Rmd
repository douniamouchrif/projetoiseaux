...
## Top 3 des espèces d'oiseaux la plus présente selon l'année
...
```
On remarque que l'espèce d'oiseaux la plus présente sur les années 2018 et 2019 est le "Sturnus Vulgaris", et la plus présente sur les années 2022 et 2023 est le "Passer Domesticus". Observons de plus près leurs traits spécifiques, ainsi qu'aux autres espèces figurant dans les top 3 de ces 4 années.
...
```
On voit que, parmis les espèces d'oiseaux figurant dans le top 3 de chaque année, chaque espèce possède comme milieu favori un milieu "généraliste" à part l'espèce "Passer domesticus" dont son milieu favori est le milieu du "bâti".
...
```
Concernant le régime alimentaire de ces espèces, il varie entre un régime mixte, végétarien et carnivore pour l'espèce "Parus major" figurant top 2 en 2019.
...
```
Du côté de la nidification, celle ci varie entre "cavité", "buisson", et "arbre" pour l'espèce "Columba palumbus". Notons que les espèces figurant top 1 sur les 4 années, à savoir l'espèce "Passer domesticus" et l'espèce "Sturnus vulgaris", ont tous deux le même lieu de nidification : "cavité".
...
```
Pour finir, la moitié des espèces figurant dans le top 3 des 4 années sont sous protection, et l'autre moitié ne l'est pas. 
Notons que la France est le pays de l'UE qui chasse le plus grand nombre d'espèces d'oiseaux, dont un total de 20 espèces qui sont de plus, menacées de disparition. (source : https://www.lpo.fr/decouvrir-la-nature/conseils-biodiversite/conseils-biodiversite/juridique/oiseaux-menaces-et-chasses-en-france#:~:text=La%20Directive%20Oiseaux%2C%20dont%20la,membres%20de%20l'Union%20Européenne)

## Graphe du nombre d'espèces par température
```{r}
...
plot(donnees_temp$Temperature, donnees_temp$Nom_Latin, type = "b", 
     ...
     col = "blue", pch = 19)
```
Nous remarquons que, pour un nombre maximal d'espèces présentes, la température optimale enregistrée est entre 5°C et 15°C avec un pic d'environ 80 espèces d'oiseaux différentes pour 10°C. Il y a cependant une chute d'espèces différentes lorsque la température se situe entre 0°C et 5°C avec bien moins de 50 espèces différentes. Lorsque la température enregistrée est de 20°C, bien qu'il y ait moins d'espèces différentes, il y en a tout de même environ 50 différentes, ce qui reste assez diversifié. 

## Graphe du nombre d'espèces par intensité du vent
```{r}
...
plot(donnees_vent$Vent_num, donnees_vent$Nom_Latin, type = "b", 
     ...
     main = "Nombre d'espèces par intensité du vent",
     col = "blue", pch = 19)
```
Notons que 0 correspond à "Calme", 1 correspond à "Très légère brise", 2 à "Légère brise" et 3 à "Petite brise", on remarque que plus l'intensité du vent augmente, moins il y a d'espèces d'oiseaux différentes, même s'il s'agit en réalité d'une légère baisse : environ 80 espèces enregistrées lors d'une "Très légère brise" contre environ 60 espèces enregistrées lors d'une "Petite brise".

## Histogramme du nombre d'oiseaux dont l'espèce est la plus présente par température et par année
```{r}
...
```
Remarquons qu'une espèce est présente dans chaque année : le "Sturnus vulgaris" (= étourneau sansonnet). Elle est le plus présente en 2018 pour les températures 5°C, 10°C et 15°C, en 2019 pour 5°C et 20°C, en 2022 pour 0°C et en 2023 pour 15°C. Cela peut s'expliquer par le fait que cette espèce possède une partie sédentaire et une autre partie, celle par exemple nord-européenne, réalise une migration annuelle lors de l'automne en direction du Sud.

L'autre espèce assez présente au cours des années est le "Passer domesticus" (= moineau domestique) en 2019 pour 0°C, en 2022 pour 5°C, 10°C, 15°C et 20°C et en 2023 pour 5°C, 10°C et 20°C. Cela peut s'expliquer par le fait que c'est un des rares oiseaux à être présent tout le long de l'année en grand nombre car il s'agit d'un oiseau sédentaire donc les migrations sont assez limitées.
...
```
Nous retrouvons ici les 2 espèces qui occupent nos histogrammes en grande partie, en notant le fait que le "Passer domesticus" est le plus présent quelque soit l'intensité du vent en 2022, et en très grande partie en 2023 aux côtés du "Sturnus vulgaris", qui lui aussi occupe une très grande partie en 2018.
...
```
Sans grande surprise, nous retrouvons une nouvelle fois nos histogrammes occupés en majeure partie par le "Sturnus vulgaris" et le "Passer domesticus" en ayant à la fois presque le même type de répartition selon la couverture nuageuse que l'intensité du vent.

Cela n'est finalement pas étonnant de retrouver ces deux espèces très présentes lors de ces années et en fonction des températures, de l'intensité du vent et de la couverture nuageuse étant les deux espèces figurant top 1 sur les 4 années.
...
ggplot(boxplot_oiseaux, aes(x = factor(Year), y = Total_Birds)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
On remarque que la majeure partie des espèces au fil des années contient un nombre d'oiseaux entre 0 et 50 oiseaux par espèces. Cependant, quelques espèces comportent plus d'oiseaux que la majeure partie, allant jusqu'à 250 oiseaux par espèces enregistrés. Enfin, la médiane se trouve environ à la même valeur pour chaque année 2018, 2019, 2022 et 2023.
...
On remarque ici aussi un pic au niveau des 10% d'artificialisation puis un résultat assez constant. Ce qui peut parraitre surprenant c'est qu'avec 0% d'artificialisation on avait environ 25 espèces alors qu'avec presque 30% d'artificialisation, on a environ 26 espèces.
...
```
Changement dans les températures qui a impacté l'observations des oiseaux -> plus chaud plus tôt donc des oiseaux qui avaient migré n'étaient pas encore revenus et inversement.
...
```
Ce graphique est difficile à interpréter, en effet, plus il y a de vent moins on entendra les oiseaux. 
...
ggplot(data = resultats_df_D_3, aes(x = Année, y = Moyenne_D_3)) +
  ...
  theme_minimal()
```
On remarque que, entre 2018 et 2019, l'indice de Simpson diminue d'environ 0.5, puis augmente d'environ 0.8 en 2022, ce qui reste des variations moyennes assez légères. De 2022 à 2023, l'indice de Simpson diminue d'environ 1.2 en moyenne, ce qui reste aussi assez léger. Finalement, l'indice de Simpson reste, lui aussi, assez constant au fil des années.
...
La forme de la relation est-elle similaire si on considère non plus l’imperméabilisation du sol mais plutôt la distance au centre ville, sachant que ces deux métriques sont corrélées? On pourra considérer Pey-Berland le centre ville et utiliser le package sf.
...
## Carte nombre et repartition des oiseaux en fonction de leur milieu favori
```{r}
data_join_1 <- Oiseaux %>%
  left_join(Traits, by = c("Nom_Latin" = "Nom.latin"))
...
## Carte nombre et repartition des oiseaux en fonction de leur regime alimentaire
```{r}
data_join_1 <- Oiseaux %>%
  left_join(Traits, by = c("Nom_Latin" = "Nom.latin"))
...
## Histogramme Distribution de MOS11 par Milieu Favori et Régime Alimentaire
```{r}
data_join_1 <- Oiseaux %>% 
  left_join(Traits, by = c("Nom_Latin" = "Nom.latin"))
...
Les résultats de l'analyse de variance (ANOVA) indiquent que la variable MOS11 présente des différences significatives en fonction de Milieu_favori (p = 0.00370) et de l'interaction entre Milieu_favori et Régime.alimentaire (p = 0.00768). Cela signifie qu'il existe des variations significatives de MOS11 entre les différents niveaux de Milieu_favori et que cette relation est influencée par Régime.alimentaire.

Cependant, la variable Régime.alimentaire seule n'a pas un effet significatif sur MOS11 (p = 0.74679).

En résumé, les résultats suggèrent que Milieu_favori et l'interaction entre Milieu_favori et Régime.alimentaire ont un impact significatif sur MOS11, tandis que Régime.alimentaire seul n'a pas un effet significatif.
...
Dans l'ensemble, le modèle semble avoir une faible capacité prédictive, comme en témoigne le faible coefficient de détermination multiple ajusté (Adjusted R-squared) de 0.02455. Cela signifie que les variables incluses dans le modèle n'expliquent qu'une petite proportion de la variance de MOS11.


## biodiversité et catégorie d'usage des sols
