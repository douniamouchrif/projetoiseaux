...
```
## Boxplot sur la distribution du nombre d'oiseaux par espèce au fil des années
```{r}
...
```
## Histogramme du nombre d'espèces différentes en fonction du niveau de vent sur toutes les années ...
```{r}
...
```
## ... et par année (2018-2019-2022-2023)
```{r}
res_vent <- data_join %>%
  filter(Year == 2018) %>%
  group_by(Vent) %>%
...
```
## Histogramme du nombre d'espèces différentes en fonction de la couverture nuageuse sur toutes les années ...
```{r}
...
## Tests des indices de Shannon et de Simpson
```{r}
# Tests fictifs
x <- c(0, 1, 1, 2, 5)
...
```{r}
# Tests sur une espèce d'oiseaux
...
```
## Représentations des indices de Shannon et de Simpson
```{r}
