...
### Formule 1
```{r}
x_ij <- aggregate(Oiseaux$Denombrement_min, by = list(Code_Maille = Oiseaux$Code_Maille, Nom_Latin = Oiseaux$Nom_Latin), FUN = sum) # changer Oiseaux pour l'espèce qu'on veut pour tester
x_tot_j <- aggregate(Oiseaux$Denombrement_min, by = list(Code_Maille = Oiseaux$Code_Maille), FUN = sum)
...
proportions <- data.frame(Code_Maille = x_ij_merge$Code_Maille, 
                             ...
                             Proportion = p_ij) # proportion de l'espèce dans le site
proportions
```
...
```{r}
H_j <- numeric(length(unique(x_ij$Code_Maille)))

for (j in 1:length(unique(x_ij$Code_Maille))) {
  p_ij_site <- p_ij[x_ij$Code_Maille == unique(x_ij$Code_Maille)[j]]
  H_j[j] <- -sum(p_ij_site * log(p_ij_site)) 
}
D_2 <- data.frame(Code_Maille = unique(x_ij$Code_Maille), D_2 = exp(H_j))
D_2
#------
H_j <- aggregate(x_ij_merge$x.x, by = list(Code_Maille = x_ij_merge$Code_Maille, Nom_Latin = x_ij_merge$Nom_Latin), FUN = sum)
#H_j
Entropie <- - H_j$x * log(H_j$x)
Entropie
Entropie <- aggregate(H_j$x, by = list(Code_Maille = H_j$Code_Maille), FUN = sum) 
Entropie
D_2 <- data.frame(Code_Maille = x_ij_merge$Code_Maille, Entropie = exp(Entropie))
D_2
```
...
```{r}
D_3 <- numeric(length(unique(x_ij$Code_Maille)))
...
### Espece : Erithacus rubecula
```{r}
# Formule 1 - test
test <- Oiseaux[Oiseaux$Nom_Latin == 'Erithacus rubecula', ]

x_ij_test <- aggregate(test$Denombrement_min, by = list(Code_Maille = test$Code_Maille, Nom_Latin = test$Nom_Latin), FUN = sum)

total_par_site <- aggregate(Oiseaux$Denombrement_min, by = list(Code_Maille = Oiseaux$Code_Maille), FUN = sum)

x_ij <- merge(x_ij_test, total_par_site, by = "Code_Maille")
x_ij
p_ij <- x_ij$x.x / x_ij$x.y
proportions_df <- data.frame(Code_Maille = x_ij$Code_Maille, 
                              Proportion_Erithacus_rubecula = p_ij)
proportions_df
```

```{r}
# Formule 2 - test
H_j <- aggregate(x_ij$x.x, by = list(Code_Maille = x_ij$Code_Maille, Nom_Latin = x_ij$Nom_Latin), FUN = sum)

Entropie <- - H_j$x * log(H_j$x)
D_2 <- data.frame(Code_Maille = x_ij$Code_Maille, Entropie = exp(Entropie))
D_2
```
...
Graph "Diversité en fonction du % sol impérméabilité" - Pas fini !!

```{r}
# Join tables Oiseaux et Traits
data_join_1 <- Oiseaux %>% left_join(y = Traits, 
...
###on join les df Land_cartoISea_500 et Oiseaux 
data_join_2 <- data_join_1 %>% left_join(y = Land_cartoISea_500, 
                                  by = c("Code_Maille" = "ID"))
data_join_2
...
donnees_agregate2 <- aggregate(data_join_2$Denombrement_min, 
                               by = list(MOS11 = data_join_2$MOS11, Year = data_join_2$Year, Milieu_favori = data_join_2$Milieu_favori), 
                               FUN = sum)

donnees_agregate2 <- transform(donnees_agregate2, nb_oiseaux = x)

# Tracer le graphique avec ggplot2
ggplot(data = donnees_agregate2, aes(x = MOS11, y = nb_oiseaux, color = Year)) +
  geom_point() +  # Points de données
  geom_smooth(method = "auto", se = TRUE, color = "black") +  # Lignes de régression globales
  geom_smooth(method = "lm", se = FALSE, aes(group = Milieu_favori), linetype = "dashed") + 
  xlab("Surfaces artificialisées en % (MOS11)") +
  ylab("Nombre d'oiseaux") +
  ggtitle("Nombre d'oiseaux en fonction du pourcentage d'artificialisation") +
  scale_color_continuous(name = "Year")  # Légende pour l'année
