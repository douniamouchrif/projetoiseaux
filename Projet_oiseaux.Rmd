...
```
On remarque que l'espèce d'oiseaux la plus présente sur les années 2018 et 2019 est le "Sturnus Vulgaris", et la plus présente sur les années 2022 et 2023 est le "Passer Domesticus". Observons de plus près leur traits spécifiques, ainsi qu'aux autres espèces figurant dans les top 3 de ces 4 années.
...
Carte nombre de oieseux et leur milieu favori
```{r}
data_join_1 <- Oiseaux %>%
  left_join(Traits, by = c("Nom_Latin" = "Nom.latin"))
...
pivot_data <- spread(aggregated_data, Milieu_favori, count, fill = 0)

aggregated_data <- na.omit(aggregated_data)

pivot_data <- spread(aggregated_data, Milieu_favori, count, fill = 0)

pivot_data$total <- rowSums(select(pivot_data, -X.x, -Y))
...
Histogramme nombre de oiseaux pour chaque regime alimentaire en fonction du milieu favori 
```{r}
data_join_1 <- Oiseaux %>% 
  left_join(Traits, by = c("Nom_Latin" = "Nom.latin"))
