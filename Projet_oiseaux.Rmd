...
### Formule 1
```{r}
oiseaux_par_site = table(Oiseaux$Code_Maille)

x_ij = aggregate(Oiseaux$Denombrement_min, by = list(Code_Maille = Oiseaux$Code_Maille, Nom_Latin = Oiseaux$Nom_Latin), FUN = sum)

p_ij <- x_ij$x / ave(x_ij$x, x_ij$Code_Maille, FUN = sum)
p_ij
```
...
## Tests
```{r}
#avec 2/3 espèces faire les calcul et voir si on trouve bien les bons trucs
test <- Oiseaux$Denombrement_min[Oiseaux$Nom_Latin == 'Erithacus rubecula']
test
```
