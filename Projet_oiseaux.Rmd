...
```
On remarque une forme de parabole. Autour des 10°C, c'est à cette dernière qu'on observe le plus grand nombre d'espèces d'oiseaux (environ 64).
```{r}
...
```
On retrouve une forme de parabole qu'on observait déjà en 2018, cependant, on voit que le nombre d'espèces à 20°C est bien supérieur à celui de 2018. On retrouve un pic à 15°C qu'on avait perdu en 2019.
...
```
Finalement en 2023, on a encore cette forme de parabole mais qui s'atténue. En effet, on a un pic à 10°C mais la descente est ensuite plus douce, à 20°C, on a donc plus d'espèces d'oiseaux en 2023 qu'en 2018.
...
```
On remarque que le vent n'a pas un impact significatif dans l'évolution du nombre d'espèces d'oiseaux. 
...
Le vent semble être fortement corrélée avec la deuxième dimension et, dans une moindre mesure, avec la première dimension.
...
for (annee in annees) {
  ...
  resultats_D_2_v1[[as.character(annee)]] <- moyenne_D_2
}

resultats_df <- data.frame(Année = as.integer(names(resultats_D_2_v1)), Moyenne_D_2 = unlist(resultats_D_2_v1))

ggplot(data = resultats_df, aes(x = Année, y = Moyenne_D_2)) +
  geom_line(color = "pink", size = 1.5) +  # Ligne rose
  geom_point() +
...
ggplot(data = D_2_annees, aes(x = Année, y = D_2, group = Code_Maille, color = factor(Code_Maille))) +
  geom_line() +
...
ggplot(data = D_3_annees, aes(x = Année, y = D_3, group = Code_Maille, color = factor(Code_Maille))) +
  geom_line() +
  labs(title = "Évolution de l'indice de simpson par sites au cours des années",
       x = "Année",
       y = "D_2") +
  theme_minimal() +
  theme(legend.position = "none")
...
Graph "Diversité en fonction du nombre d'oiseaux et le degré d'artificialisation"
```{r}
data_join_1 <- Oiseaux %>% 
...
donnees_agregate2 <- aggregate(Denombrement_min ~ MOS11 + Year + Milieu_favori, data = data_join_2, FUN = sum)

donnees_agregate2$Year <- factor(donnees_agregate2$Year)

ggplot(data = donnees_agregate2, aes(x = MOS11, y = Denombrement_min, color = Milieu_favori)) +
  ...
  ggtitle("Diversité en fonction du nombre d'oiseaux et le degré d'artificialisation") +
...
donnees_agregate_distance <- aggregate(Denombrement_min ~ distance_centre_ville, data = centres_villes, FUN = sum)

ggplot(data = donnees_agregate_distance, aes(x = distance_centre_ville, y = Denombrement_min)) +
  geom_point(color = "grey") +
  geom_smooth(method = "auto", se = FALSE, color = "black") + 
  xlab("Distance par rapport au centre-ville") +
  ylab("Nombre d'oiseaux") + 
  ggtitle("Nombre d'oiseaux en fonction de la distance par rapport au centre-ville") +
  theme_minimal()
```
...
donnees_agregate <- aggregate(Denombrement_min ~ Nom_lieu + X.y + Y + BufferSize + name_site, data = data_join_2, FUN = sum)

palette <- colorNumeric(palette = "viridis", domain = donnees_agregate$Denombrement_min)

amp_oiseaux <- leaflet(data = donnees_agregate) %>% 
  addTiles() %>%
  addCircles(~X.y, ~Y, weight = 1, radius = ~BufferSize, color = ~palette(Denombrement_min), opacity = 1, fillOpacity = 0.5, popup = ~name_site)

amp_oiseaux <- amp_oiseaux %>% addLegend(position = "bottomright", pal = palette, values = ~Denombrement_min, title = "Denombrement_min", opacity = 1)
amp_oiseaux
```
