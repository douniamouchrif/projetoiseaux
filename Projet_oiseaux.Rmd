...
Interesant d'etudier MOS 4 et 5. Faire une ACP aussi sur ces variables.
...
centre_ville_interet <- data_join[data_join$name_site == "Bordeaux-A", ]

centre_ville_latitude <- data_join$X.y[data_join$name_site == "Bordeaux-A"]
centre_ville_longitude <- data_join$Y[data_join$name_site == "Bordeaux-A"]

centre_ville_latitude <- centre_ville_latitude[1]
centre_ville_longitude <- centre_ville_longitude[1]

data_join$Y <- as.numeric(data_join$Y)
centre_ville_latitude <- as.numeric(centre_ville_latitude)

data_join$distance_centre_ville <- sqrt((data_join$X.y - centre_ville_longitude)^2 + (data_join$Y - centre_ville_latitude)^2)
...
data_acp <- data_final[, c("Nb_espece", "distance_centre_ville", paste0("MOS", 1:14))]
...
palette <- colorNumeric(palette = "viridis",domain = table_jointe_tbl$MOS5,reverse = TRUE)

amp_artificial <- leaflet(data = table_jointe_tbl) %>% 
  addTiles() %>%
  addCircles(~X.y, ~Y, weight = 1, radius = ~BufferSize, color = ~palette(MOS5), opacity = 1, fillOpacity = 0.5, popup = ~name_site)

amp_artificial <- amp_artificial %>% addLegend(position = "bottomright", pal = palette, values = ~MOS5, title = "MOS5", opacity = 1)
