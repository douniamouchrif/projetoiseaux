...
for (j in 1:length(unique(x_ij$Code_Maille))) {
  ...
  H_j[j] <- ifelse(all(p_ij_site == 0), 0, -sum(p_ij_site * log(p_ij_site)))
}
...
```{r}
Code_Maille_unique <- unique(x_ij$Code_Maille)
D_3 <- numeric(length(unique(x_ij$Code_Maille)))

for (j in 1:length(unique(x_ij$Code_Maille))) {
  p_ij_site <- p_ij[x_ij$Code_Maille == unique(x_ij$Code_Maille)[j]]
   D_3[j] <- ifelse(all(p_ij_site == 0), 0, 1 / sum(p_ij_site^2))
}
D_3_df <- data.frame(Code_Maille = Code_Maille_unique, D_3 = D_3)
D_3_df
```
...
D_2 <- data.frame(Code_Maille = x_ij$Code_Maille, exp_Entropie = exp(Entropie))
D_2
```
...
Graph "Diversité en fonction du nombre d'oiseaux et le degré d'artificialisation" pour la période 2018-2023
```{r}
data_join_1 <- Oiseaux %>% 
